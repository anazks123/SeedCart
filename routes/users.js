...
router.get('/', function(req, res, next) {
  ...
  var sql5 = "select * from product where catagory = 'frutes'"
  if(req.session.user){
        ...
        //  res.redirect('/users/error')
            ...
            con.query(sql3,(err,offer)=>{
              if(err){
                ...
                con.query(sql5,(err,frute)=>{
                  if(err){
                    res.send(err)
                   // res.redirect('/users/error')
                  }else{
                  
                    res.render('user/home',{product,user,CartTotal,obj1,obj2,obj3,frute});
                  }
                })
...
con.query(sql,data,(err,result)=>{
  if(err){
    ...
    let mailOptions = {
      from: 'Green cartTeam',
      to: email,
      subject: 'PeyMent Invoice',
      text: `Your Item Was successfully Ordered from GreenCart Team
        Total Amount:${amount}
        Product ID:${Pid}
        Seller ID:${sellerid}

        Authorization :GreenCart Team
...
router.get('/offers',(req,res)=>{
  ...
  con.query(sql,(err,result)=>{
    if(err){
      ...
      var user = req.session.user;
      res.render('user/offer',{offer:result,user,homepage:true})
    }
  })
 ...
 var data = req.body.cata;
  ...
  con.query(sql,[data],(err,result)=>{
    if(err){
      ...
      var user = req.session.user;
      res.render('user/catagory',{data:result,user,homepage:true})
    }
  })
